namespace Glide
{
    public partial class Tween
    {
        ...
        /// </summary>
        public float Completion { get { var c = this.time / this.Duration; return c < 0 ? 0 : (c > 1 ? 1 : c); } }
        ...
        private void Update(float elapsed)
        {
            if (this.firstUpdate)
            {
                ...
                while (i-- > 0)
                {
                    if (this.lerpers[i] != null)
                        this.lerpers[i].Initialize(this.start[i], this.end[i], this.behavior);
                }
            }
            else
            {
                ...
                if (this.time == 0 && this.timesRepeated == 0 && this.begin != null)
                    this.begin();
                ...
                if (this.time == 0 && this.behavior.HasFlag(Lerper.Behavior.Reflect))
                    this.Reverse();

                if (this.update != null)
                    this.update();

                if (doComplete && this.complete != null)
                    this.complete();
            }
        }
        ...
        public Tween From(object values)
        {
            ...
            for (var i = 0; i < props.Length; ++i)
            {
                ...
                var info = new GlideInfo(this.Target, property.Name, true);
        ...
        {
            if (this.begin == null) this.begin = callback;
            else this.begin += callback;
            return this;
        }
        ...
        {
            if (this.complete == null) this.complete = callback;
        ...
        {
            if (this.update == null) this.update = callback;
